...
# Modify the torch.load calls to include weights_only=True
# This addresses the FutureWarning

# Example modification in train_best_model function
def train_best_model(trial):
    ...
    # Optionally, load existing checkpoint if it matches the model architecture
    checkpoint_path = 'checkpoints/best_model_final.pth'
    if os.path.exists(checkpoint_path):
        try:
            ...
            logging.info("Proceeding without loading checkpoint.")
    else:
        logging.info(f"No existing checkpoint found at {checkpoint_path}. Starting fresh.")

    # Get dataloaders
    train_loader, val_loader = get_dataloaders(batch_size, img_size)

    # Rest of your training code...
    # Ensure all torch.load calls have weights_only=True
    # Example:
    # When loading the best model after training
    if isinstance(model, nn.DataParallel):
