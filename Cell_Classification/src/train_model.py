...
# Filter the dataset to only include samples with label 0
indices = [i for i, (_, label) in enumerate(train_dataset) if label == 0]
train_dataset_filtered = Subset(train_dataset, indices)
train_dataloader_filtered = DataLoader(train_dataset_filtered, batch_size=16, shuffle=True)
