...
def objective(trial):
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(skf.split(indices, labels)):
        ...
        for epoch in range(num_epochs):
            ...
            custom_score_test = calculate_custom_score(test_targets, preds_binary)    
            print(f"Custom Score: {custom_score_test:.4f}")
            logging.info(f"Custom Score ON TEST AFTER EPOCH: {custom_score_test:.4f}")
    ...
    if avg_custom_score > best_custom_score:
        ...
        ground_truths = np.array(test_targets)
        ...
        print(f"Custom Score: {custom_score_test:.4f}")
        logging.info(f"Custom Score ON TEST AFTER TRIAL: {custom_score_test:.4f}")
        ...
        custom_score_val = calculate_custom_score(val_targets, val_preds)
        ...
        print(f"Recall   : {recall:.4f}")
        print(f"F1 Score : {f1:.4f}")
        print(f"Custom Score: {custom_score_val:.4f}")
        logging.info(f"Custom Score ON VAL AFTER TRIAL: {custom_score_val:.4f}")
