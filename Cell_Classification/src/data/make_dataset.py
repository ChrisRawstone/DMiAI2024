...
class LoadTifDataset(Dataset):
    def __init__(self, csv_file, image_dir, transform=None):
        ...
        self.labels_df = pd.read_csv(csv_file,  dtype={'image_id': int})
    ...
    def __getitem__(self, idx):
        if self.image_dir == 'data/train_augmentation/':
            img_name = str(self.labels_df.iloc[idx, 0]).zfill(4)  # Zero-pad to 3 digits
        else: 
