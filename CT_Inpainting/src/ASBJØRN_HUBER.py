...
def visualize_and_log(inputs, outputs, labels, epoch):
    ...
    plt.tight_layout()
    plt.savefig(f'plots/epoch_{epoch + 1}_reconstruction_huber.png')
    plt.show()
...
def save_model(model):
    ...
    torch.save(model.state_dict(), f'models/ct_inpainting_unet_huber{timestamp}.pth')
...
class CTInpaintingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Create a second mask but only for parts that overlap with tissue
        ...
        if self.transform:
            ...
            mask_rotated = self.transform(mask)
            if num_rotations > 0:
                mask_rotated = torch.rot90(mask_rotated, num_rotations, [1, 2])
...
class UNet(nn.Module):
    ...
    def conv_block(self, in_channels, out_channels, dropout_prob=0.3):
        block = nn.Sequential(
...
if __name__ == "__main__":
  ...
  criterion = nn.HuberLoss()  # MAE # Maybe try Huber loss
  ...
  best_model_path = 'models/ASBJØRN_LAST_ATTEMPT_BEST_HUBER.pth'  # Path to save the best model
